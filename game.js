...
const CONFIG = {
  ...
  PLANT_COOLDOWN: {
    ...
    peashooter: 5000,      // 5秒
    wallnut: 35000,        // 35秒（高血量防御）
    snowpea: 7500,         // 7.5秒
    cherrybomb: 30000,     // 30秒
    potatomine: 10000,     // 10秒
    jalapeno: 20000,       // 20秒
    chomper: 12000,        // 12秒
    repeater: 7500         // 7.5秒
  },
  ...
  PEA_SHOOT_INTERVAL: 900,      // 豌豆射击间隔(900ms ≈ 每秒1.1颗)
  PEA_SPEED: 4,                 // 豌豆移动速度（降低20%）
  PEA_DAMAGE: 20,               // 豌豆伤害
...
// ANIMATIONS - 定义每个植物的动画序列
;

// PlantSprite 类 - 管理植物的逐帧动画
...
function updateChompers() {
  gameState.plants.forEach(plant => {
    ...
    for (const zombie of gameState.zombies) {
      ...
      // 用 getBoundingClientRect 保证坐标系一致
      const zombieRect = zombie.element.getBoundingClientRect();
      const zombieCenterX = zombieRect.left + zombieRect.width / 2;
      
      // 触发范围：僵尸中心距离植物中心 150px 内
      const dist = plantCenterX - zombieCenterX; // 正数=僵尸在植物右边
      if (dist >= -150 && dist <= 0) {
        if (Math.random() < 0.01) {
          console.log(`大嘴花[${plant.row},${plant.col}]检测: 植物X=${plantCenterX.toFixed(0)}, 僵尸X=${zombieCenterX.toFixed(0)}, 距离=${dist.toFixed(0)}`);
        }
...
function updateRepeaters() {
  ...
  gameState.plants.forEach(plant => {
    ...
    // 发射双倍豌豆（调用 shootPea，与豌豆射手共用同一函数）
    for (let i = 0; i < 2; i++) {
      setTimeout(() => {
        if (plant.element) {
          shootPea(plant);
...
function updatePotatoMines() {
  ...
  gameState.plants.forEach(plant => {
    ...
    for (const zombie of gameState.zombies) {
      ...
      // 统一用 getBoundingClientRect 获取页面坐标
      const zRect = zombie.element.getBoundingClientRect();
      
      // 触发条件：僵尸任何部分进入格子范围（放宽判定）
      const zombieLeft = zRect.left;
      const zombieRight = zRect.right;
      
      // 调试日志（采样输出，避免刷屏）
      if (Math.random() < 0.01) {
        console.log(`土豆地雷[${plant.row},${plant.col}]检测: 格子中心=${cellCenterX.toFixed(0)}, 僵尸[${zombieLeft.toFixed(0)}, ${zombieRight.toFixed(0)}]`);
      }
...
function attackPlant(zombie, plant) {
  ...
  if (now - zombie.lastAttackTime >= 1000) {
    ...
    // 坚果墙受击动画
    if (plant.type === 'wallnut' && plant.element && plant.element.sprite) {
...
function spawnWave() {
  ...
  // 根据关卡配置计算僵尸数（从第一波递增）
  // 第一波少，后面逐渐多
  let waveZombies = Math.max(1, Math.floor(gameState.currentWave * 0.8 + 1));
  ...
  // 波次内出怪间隔
  const spawnDelay = Math.max(1000, 2000 - gameState.currentWave * 100);
