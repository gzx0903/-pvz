...
const CONFIG = {
  ...
  PLANT_HP: {
    sunflower: 100,
    peashooter: 100,
    wallnut: 1600,          // 加强坚果血量（原400，现1600，再次翻倍）
    snowpea: 100,
    cherrybomb: 100,
    potatomine: 100,
    jalapeno: 100,
  ...
  PEA_SHOOT_INTERVAL: 500,      // 豌豆射击间隔(500ms = 每秒2颗)
  PEA_SPEED: 5,                 // 豌豆移动速度
  PEA_DAMAGE: 20,               // 豌豆伤害
...
function renderPlant(row, col, plantType) {
  ...
  // 逐帧精灵动画（不支持的类型用单张PNG兜底）
  if (SPRITE_PARTS[plantType]) {
    plant.sprite = new PlantSprite(plantType, plant);
  } else if (PLANT_IMAGES[plantType]) {
...
function gameLoop() {
  ...
  updatePlantAnimations(performance.now());
  updateSuns();
  updateSunflowers();
  updatePeashooters();
  updateRepeaters();
...
const SUNFLOWER_PROD_INTERVAL = 18000; // 向日葵每18秒产一个阳光
...
function produceSunFromSunflower(plant) {
  ...
  if (plant.sprite) plant.sprite.play('produce');
...
function shootPea(plant) {
  ...
  if (plant.sprite) plant.sprite.play('shoot');
...
function hitZombie(zombie, damage, isSnowpea) {
  ...
  // 击退效果
  zombie.x += 5;
  if (zombie.element) zombie.element.style.left = zombie.x + 'px';
...
function chomperChew(plant, zombie) {
  ...
  plant.chewing = true;
  plant.element.innerHTML = CHOMPER_SVG_CHEWING;
  ...
  const chewInterval = setInterval(() => {
    if (!plant.element || !zombie.element || zombie.hp <= 0) {
      clearInterval(chewInterval);
      plant.chewing = false;
      plant.currentTarget = null;
      if (plant.element) {
        plant.element.innerHTML = CHOMPER_SVG_NORMAL;
      }
      return;
    }
    
    zombie.hp -= CONFIG.CHOMPER_DAMAGE;
    if (zombie.hp <= 0) {
      zombie.element.remove();
      gameState.zombiesKilled++;
      clearInterval(chewInterval);
...
function updateRepeaters() {
  ...
  gameState.plants.forEach(plant => {
    ...
    for (let i = 0; i < 2; i++) {
      setTimeout(() => {
        if (plant.element) {
          createBullet(plant.row, plant.col, 'pea');
          playSound('shoot');
        }
      }, i * 100);
    }
...
const ZOMBIE_TYPES = {
  normal: { hp: 100, speed: 0.4, damage: 20, icon: 'zombie-normal', minWave: 1 },
  cone: { hp: 200, speed: 0.35, damage: 25, icon: 'zombie-cone', minWave: 3 },
  bucket: { hp: 400, speed: 0.3, damage: 30, icon: 'zombie-bucket', minWave: 5 },
  flag: { hp: 100, speed: 0.4, damage: 20, icon: 'zombie-flag', minWave: 1 },
  polevault: { hp: 150, speed: 0.55, damage: 25, icon: 'zombie-polevault', minWave: 6, canJump: true },
  newspaper: { hp: 250, speed: 0.25, damage: 25, icon: 'zombie-newspaper', minWave: 7, enraged: false },
  screenDoor: { hp: 700, speed: 0.2, damage: 30, icon: 'zombie-screenDoor', minWave: 8, hasShield: true },
  football: { hp: 500, speed: 0.5, damage: 35, icon: 'zombie-football', minWave: 9 }
};
